...
# Пулы Clean IP Cloudflare по странам
COUNTRY_ENDPOINTS = {
    "de": {
        "name": "🇩🇪 Германия",
        "clean_ips": [
...
async def register_warp_account(country_code="de"):
    """Регистрация профиля в API Cloudflare."""
    ...
    async with httpx.AsyncClient(http2=True, headers=HEADERS, timeout=12.0) as client:
        try:
            ...
            # Жёстко задаём порт 53 для максимального пробития фильтрации
            return {
...
def build_amnezia_wg_config(warp_data):
    """Сборка конфига с адаптивными параметрами под провайдеров РФ."""
    if not warp_data: 
        return None
